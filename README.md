...
- Dockerfile を作成し、Flask API をコンテナで起動
- docker compose を使い、起動・停止をコマンド一つで管理
- SQLite を volume 経由で保持し、コンテナ再起動でもデータが消えない状態を確認

**何を・何のためにしたか**

- Flask バックエンドを Docker コンテナ化した
- 開発環境をローカル環境依存にせず、再現可能にするため
- 今後 DB を MySQL などに置き換える前段階として導入

**初めて Docker を触った感想**

- 「アプリを 1 つの箱にまとめて動かす」感覚が理解できた
- ローカルで動いていた Flask が、そのまま別環境で動くことに驚いた
- エラーは多かったが、ログと状態を切り分けて考える重要性を学んだ
